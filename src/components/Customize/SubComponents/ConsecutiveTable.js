...
function ConsecutiveTable() {
  ...
  useEffect(() => {
      ...
      const results = dataList.filter(dataList =>
          dataList.cs1.subject.toLowerCase().includes(searchTerm)
      );
        ...
        <div className="form-group">
          ...
          <div className="row col-md-12">
            <Col sm="6 pb-0 text-left">
              <Link to={"/addConsecutive"}>
                <input type="submit" value="Add Consecutive Sessions" className= "btn btn-primary"/>
              </Link>
            </Col>
        ...
        <Table className={classes.table} aria-label='simple table'>
          ...
          >
            <TableRow align='center'>
              <StyledTableCell align='center'>Consecutive Session 1</StyledTableCell>
              <StyledTableCell align='center'>Consecutive Session 2</StyledTableCell>
              <StyledTableCell align='center'>Consecutive Session 3</StyledTableCell>
            ...
            {searchResults.map((item, key) => (
              <TableRow hover key={key}>
                <TableCell align='center'>
                    {item.cs1.subject} {item.cs1.subjectCode}<br/>
                    {item.cs1.tag}<br/>
                    {item.cs1.groupId}
                </TableCell>
                <TableCell align='center'>
                    {item.cs2.subject} {item.cs2.subjectCode}<br/>
                    {item.cs2.tag}<br/>
                    {item.cs2.groupId}
                </TableCell>
                <TableCell align='center'>
                    {item.cs3.subject} {item.cs3.subjectCode}<br/>
                    {item.cs3.tag}<br/>
                    {item.cs3.groupId}
                </TableCell>
                <TableCell align='center'>
